...
interface Labels {
  ...
  maxLengthPx?: number;
  minLengthPx?: number;
  mode?: 'horizontal' | 'tilted' | 'layered';
}
...
interface ComponentFixture {
  getRenderOutput(): RenderNode[];
  simulateCreate(component: unknown, config: AxisConfig): void;
  simulateRender(opts: Opts): void;
  simulateUpdate(settings?: AxisSettings): void;
  simulateLayout(opts: Opts): unknown;
  mocks(): {
    chart: ChartMock;
    renderer: unknown;
    theme: unknown;
  ...
  findNodes(selector: string): unknown[];
}

interface ChartMock {
  scale: {
    returns(scale: unknown): void;
  };
  brush(): { on(event: string, handler: () => void): void };
  dataset: unknown;
  container: unknown;
  formatter: unknown;
  logger(): { warn: unknown };
...
type ScaleBand = ReturnType<typeof band>;
type ScaleLinear = ReturnType<typeof linear>;
type Scale = ScaleBand | ScaleLinear | Record<string, never>;
...
describe('Axis', () => {
  ...
  function verifyNumberOfNodes(type: string, expectedNbrOfNodes: number): void {
    const nodes = componentFixture.getRenderOutput().filter((n: RenderNode) => n.type === type);

    expect(nodes.length).to.equal(expectedNbrOfNodes, `Unexpected number of ${type} nodes`);
  }

  beforeEach(() => {
    componentFixture = componentFactoryFixture() as ComponentFixture;
  ...
  describe('Settings', () => {
    ...
    describe('maxLengthPx', () => {
      ...
      beforeEach(() => {
        scale = band() as ScaleBand;
        (scale as ScaleBand).domain(['AAAAAAAAAA', 'BBBBBBBBB']);
        chart.scale.returns(scale);
      ...
      it('should limit text nodes max width in horizontal mode and docked vertically', () => {
        ...
        const textNodes = componentFixture.getRenderOutput().filter((n: RenderNode) => n.type === 'text');
    ...
    describe('Defaults', () => {
      ['left', 'right', 'top', 'bottom'].forEach((d: string) => {
  ...
  describe('Lifecycle', () => {
    describe('Update', () => {
      it('should handle an update where scale type changes from discrete to continuous', () => {
        scale = band() as ScaleBand;
        (scale as ScaleBand).domain([0, 1, 2, 3, 4, 5]);
        chart.scale.returns(scale);
  ...
  describe('discrete', () => {
    beforeEach(() => {
      scale = band() as ScaleBand;
      (scale as ScaleBand).domain([0, 1, 2]);
      (scale as ScaleBand).range([0, 1]);
      chart.scale.returns(scale);
    });
