...
describe('svg renderer', () => {
  ...
  describe('appendTo', () => {
    ...
    it('should apply font smoothing', () => {
      ...
      expect(((svg.element() as any)?.style as any)['-webkit-font-smoothing']).to.equal('antialiased');
      expect(((svg.element() as any)?.style as any)['-moz-osx-font-smoothing']).to.equal('antialiased');
    });
  });
  ...
  describe('render', () => {
    ...
    it('should call tree creator with proper params', () => {
      ...
      delete ((((actual as unknown[])[0] as any)?.children?.[1]) as any)?.disabled;
      expect(actual).to.deep.equal([sceneContainer]);
      expect((treeRenderer.render as any)).to.have.been.calledWith(s.children, svg.root());
    });
  ...
  describe('clear', () => {
    it('should remove all elements', () => {
      ...
      expect((((svg.root() as any) as any).children as unknown[]).length).to.equal(2);

      svg.clear();

      expect((((svg.root() as any) as any).children as unknown[]).length).to.equal(0);
    });
